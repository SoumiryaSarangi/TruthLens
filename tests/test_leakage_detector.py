...
def test_each_leak_kind_is_detected(leaky, kind):
    found = {f.kind for f in failures(find_leakage("toy_leaky", leaky))}
...
def test_punctuation_variant_is_caught_by_simhash_not_by_exact_hash(leaky):
    ...
    """
    near = [f for f in failures(find_leakage("toy_leaky", leaky))
...
def test_clean_fixture_is_clean(clean):
    found = find_leakage("toy_clean", clean)
